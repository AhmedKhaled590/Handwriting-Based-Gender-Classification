...
parser.add_argument("--input_folder", type=str,
                    default=r"Gold_Hinge/InputImages")
parser.add_argument("--output_folder", type=str,
                    default=r"Gold_Hinge/FeaturesOutput")
...
for i, class_dir in enumerate(class_dirs):
    ...
    for img_filename in tqdm(img_filenames):
        try:
            ...
            if(class_dir == 'icdarTrainImages'):
                labels.append(img_filename[:4])
            else:
                labels.append(i)
...
np.savez(os.path.join(output_folder, f"labels"),
         label=labels, label_name=label_names)
